...
def load_image(path):
    ...
    image = np.reshape(image, ((1,) + image.shape))  # 增加 batch 维度
    image = image - MEAN_VALUES  # 减去均值
    return image

def save_image(path, image):
    # 恢复均值
    image = image + MEAN_VALUES
    image = image[0]  # 去掉 batch 维度
    # 限制像素值范围
    image = np.clip(image, 0, 255).astype('uint8')
    # 使用 Pillow 保存图像
...
with tf.Session(config=config) as sess:
	...
	# 保存路径
	# MODEL_SAVE_PATH = 'basetrans_model_checkpoint/'
	...
	for i in range(ITERATIONS):
		...
		if i % 100 == 0:
			...
			# 保存模型
			#saver.save(sess, os.path.join(MODEL_SAVE_PATH, 'style_transfer_model'), global_step=i)
		if i == 999:
			output_image_final = sess.run(model['input'])
			save_image(os.path.join(OUTPUT_DIR, 'change.jpg'), output_image_final)
			


	# 在训练结束时保存最终模型
	#saver.save(sess, os.path.join(MODEL_SAVE_PATH, 'style_transfer_model_final'))