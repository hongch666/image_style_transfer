...
print(style_images)
print('up there are styles images------------------------------------------------------------------------------------------------------------------------------------------')
...
print('up there are the training picture, the first num is the number of the picture-------------------------------------------------------------------------------------------------------------------------')


#加载vgg19模型，并定义一个函数，对于给定的输入，返回vgg19各个层的输出值，就像在GAN中那样，通过 variable_scope 重用实现网络的重用
...
#这里的数字就是对应风格的下标，每次训练换这个就行，他会自动生成对应模型到自己生成的文件夹中
...
print('there choose one of the style picture, then cut it into a square as the training picture, then expand it into a four dimension array----------------------------------------------------------------------')
...
class InstanceNorm(tf.keras.layers.Layer):
  ...
  def call(self, inputs, training=False):
      ...
      # 通过两个可学习的参数（gamma和beta）来缩放和平移归一化的结果
      gamma = tf.Variable(tf.ones([1, 1, 1, inputs.shape[-1]]))
      beta = tf.Variable(tf.zeros([1, 1, 1, inputs.shape[-1]]))
      return gamma * normalized + beta 
    
instance_norm_layer = InstanceNorm()
def instance_norm(inputs):
  return instance_norm_layer(inputs, training=True)
...
#将转换网络的输出即迁移图片，以及原始内容图片都输入到vgg19，得到各自对应层的输出计算内容损失函数
...
optimizer = optimizer.minimize(loss, var_list=vars_t)

# # 创建Adam优化器实例，并设置学习率
# optimizer = tf.keras.optimizers.Adam(learning_rate=0.001)

# # 使用优化器最小化损失函数，指定需要优化的变量列表
# optimizer.minimize(loss, var_list=vars_t)
...
tf.summary.scalar('losses/total variation_loss', total_variation_loss)
...
for e in range(epochs):
  ...
  # print('这是之前的')
  # print(result)
  # 假设result是一个numpy数组，其值域在0到255之间
  # 如果result的值域不在0到255之间，需要先进行归一化处理
  result=result*255
  result = np.clip(result, 0, 255)  # 确保值在0到255之间

  # 将浮点数转换为整数类型，如果result的值域已经是0到255，则不需要乘以255
  result = result.astype(np.uint8)

  # 构造文件路径
  file_path = os.path.join(OUTPUT_DIR, f'sample_{e}.jpg')

  # print('这是之后的')
  # print(result)

  # 保存图像
  imsave(file_path, result)
